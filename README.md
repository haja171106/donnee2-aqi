...
- `.github/workflows/etl.yml` se déclenche chaque heure via un événement `repository_dispatch` et enchaîne extraction → transformation → validation → chargement → commit des données mises à jour dans le dépôt.
...
**Déclenchement horaire : cron-job.org, sans le `schedule:` natif de GitHub.** Le déclencheur `schedule:` de GitHub Actions a été testé en premier, mais s'est avéré peu fiable en pratique : GitHub le documente lui-même comme du "best-effort", avec des retards observés allant de 1h15 à plus de 4h sous forte charge sur son infrastructure partagée, au lieu d'un déclenchement à l'heure pile. Il a été remplacé par [cron-job.org](https://cron-job.org), un service de cron externe gratuit qui appelle l'API GitHub (`POST /repos/.../dispatches`) précisément à la minute 0 de chaque heure, réveillant le workflow via l'événement `repository_dispatch`. Le `schedule:` interne a été volontairement retiré plutôt que conservé en garde-fou : les deux déclencheurs actifs en parallèle produisaient chacun leur propre run à quelques minutes d'écart (ex. un succès à 15h00 via cron-job.org, un autre à 15h03 via `schedule:`), doublant inutilement l'historique sans apporter de données supplémentaires grâce à l'idempotence du pipeline. Les étapes réseau du workflow (installation des dépendances, appel à l'API OpenWeather) retentent automatiquement jusqu'à 3 fois en cas d'échec transitoire. Le titre affiché dans l'onglet Actions (`run-name`) indique explicitement le déclencheur de chaque run (`repository_dispatch` ou `workflow_dispatch`), ce qui permet de vérifier d'un coup d'œil que la collecte est bien automatique et qu'il n'y a plus qu'un seul run par heure.
